...
#---------------------------------------------GC content------------------------------------------------------------------------------------------#
cat $trans_loop | $ref/remove_outlier.py $ref/$genome.blacklist.ends | ./remove_ends_without_HD.py hg19.frag.end.GC.map |  >$name.trans_loop.without.blacklist
...
./batch_anchor_by_chrom.pl $genome.HindIII.frag.bed $name.frag_loop frag.2.all.5kb.anchor
