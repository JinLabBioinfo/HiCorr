...
cat $cis_loop | $bin/fragdata_to_anchordata_DNPII.pl - $ref/${genome}_DPNII_frag_2_anchor  | ${bin}/remove.blacklist_DPNII.py $ref/${genome}_5kb_anchors_blacklist |${bin}/pick.dist.pl $anchorbed - | awk '{if($4<=2000000) print $0}' > end_loop.2M.rmbl &

cat $cis_loop | $bin/fragdata_to_anchordata_DNPII.pl - $ref/${genome}_DPNII_frag_2_anchor  | ${bin}/remove.blacklist_DPNII.py $ref/${genome}_5kb_anchors_blacklist |${bin}/pick.dist.pl $anchorbed - | awk '{if($4>2000000) print $0}' > end_loop.gt.2M  &

cat $trans_loop | $bin/fragdata_to_anchordata_DNPII.pl - $ref/${genome}_DPNII_frag_2_anchor  | ${bin}/remove.blacklist_DPNII.py $ref/${genome}_5kb_anchors_blacklist > end_loop.rmbl.trans &
