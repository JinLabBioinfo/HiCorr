...
use strict;
my ($regress, $frag_bed, $len_group_file, $dist_group_file, $group_stat_file) = @ARGV;
if(not defined $dist_group_file){
	die("Usage:./get_loop_lambda.pl <frag_loop_data> <frag bed> <frag_length_group> <frag_dist_group> <group_statistic_file>\n");
}
