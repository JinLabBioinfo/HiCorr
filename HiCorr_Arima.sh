...
name=$5
genome=$6
anchorbed=${ref}/${genome}_Arima_anchors_avg.bed

cat $cis_loop | $bin/fragdata_to_anchordata.pl - $ref/${genome}_Arima_frag_2_anchor | ${bin}/remove.blacklist.py $ref/${genome}_5kb_anchors_blacklist |${bin}/pick.dist.pl $anchorbed - | awk '{if($4<=2000000) print $0}' > end_loop.2M.rmbl &

cat $cis_loop | $bin/fragdata_to_anchordata.pl - $ref/${genome}_Arima_frag_2_anchor | ${bin}/remove.blacklist.py $ref/${genome}_5kb_anchors_blacklist |${bin}/pick.dist.pl $anchorbed - | awk '{if($4>2000000) print $0}' > end_loop.gt.2M  &

cat $trans_loop | $bin/fragdata_to_anchordata.pl - $ref/${genome}_Arima_frag_2_anchor | ${bin}/remove.blacklist.py $ref/${genome}_5kb_anchors_blacklist > end_loop.rmbl.trans &
