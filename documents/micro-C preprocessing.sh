...
wait 
echo $name "trans:" `cat frag_loop.$name.trans | awk '{sum+=$3}END{print sum/2}'` "cis:" `cat frag_loop.$name.cis | awk '{sum+=$3}END{print sum/2}'` "cis within 2M:" `cat frag_loop.$name.cis | awk '{if($4<=2000000)print}' | awk '{sum+=$3}END{print sum/2}'` "cis within 200K:" `cat frag_loop.$name.cis | awk '{if($4<=200000)print}' | awk '{sum+=$3}END{print sum/2}'` >> summary.total.read_count 
